...
import './App.css'
import { API_URL, CONVERT_API, QUESTIONS_API, SHOW_QUESTION_API } from './apiConfig';
...
function IntensiveTestJa() {
  ...
  // Fetch question directly by ID
  const fetchQuestionById = async () => {
    if (!form.question_id || form.question_id.trim() === '') {
      setError('問題IDを入力してください。');
      return;
    }
    ...
    setError(null);
    try {
      const url = `${SHOW_QUESTION_API}/${form.question_id.trim()}`;
      ...
      if (!res.ok) {
        ...
        if (res.status === 404) {
          throw new Error(`問題ID "${form.question_id.trim()}" が見つかりませんでした。\n別の問題IDを試すか、グレードと問題タイプから問題を選択してください。`);
        } else {
          throw new Error(`問題の取得に失敗しました (status: ${res.status}): ${errorText || 'Unknown error'}`);
        }
      }
      ...
      if (data && typeof data === 'object') {
        ...
        // Map API grade back to Japanese display format
        const reverseGradeMap = Object.fromEntries(
          Object.entries(EIKEN_GRADES_API_MAP).map(([k, v]) => [v, k])
        );
        const gradeFromApi = reverseGradeMap[data.exam_grade] || data.exam_grade;
        
        // Map API question type back to Japanese display format
        let questionTypeFromApi = '';
        if (data.question_type === 'composition') questionTypeFromApi = '作文';
        else if (data.question_type === 'summary') questionTypeFromApi = '要約';
        else if (data.question_type === 'email') questionTypeFromApi = 'Eメール';
        
        setForm(prev => ({
          ...prev,
          exam_grade: gradeFromApi,
          question_type: questionTypeFromApi,
          min_words: data.min_words ? String(data.min_words) : '',
          max_words: data.max_words ? String(data.max_words) : '',
  ...
  // Fetch a random question to help users see valid question IDs
  const fetchRandomQuestion = async () => {
    setFetchingQuestions(true);
    setError(null);
    try {
      const params = new URLSearchParams({
        exam_grade: '2', // Default to grade 2
        question_type: 'composition', // Default to composition
      });
      const url = `${QUESTIONS_API}?${params.toString()}`;
      console.log('Fetching random question from:', url);
      
      const res = await fetch(url);
      if (!res.ok) throw new Error('ランダム問題の取得に失敗しました');
      ...
      if (data && typeof data === 'object') {
        ...
        if (keys.length > 0) {
          ...
          const gradeFromApi = reverseGradeMap[q.exam_grade] || q.exam_grade;
          
          // Map API question type back to Japanese display format
          let questionTypeFromApi = '';
          if (q.question_type === 'composition') questionTypeFromApi = '作文';
          else if (q.question_type === 'summary') questionTypeFromApi = '要約';
          else if (q.question_type === 'email') questionTypeFromApi = 'Eメール';
      ...
      setError('ランダム問題の取得に失敗しました: ' + (err.message || err.toString()));
    } finally {
      setFetchingQuestions(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const selectedQuestion = questionObj ? questionObj.question : form.question;
    const minWords = questionObj ? questionObj.min_words : form.min_words;
    const maxWords = questionObj ? questionObj.max_words : form.max_words;
    const underlined = questionObj ? questionObj.underlined : form.underlined;
  ...
  return (
    <main id="root">
      ...
      {/* Question ID input and fetch button */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, padding: '12px', backgroundColor: '#f8f9fa', borderRadius: 8 }}>
        <label htmlFor="question_id" style={{ fontWeight: 500, minWidth: 80 }}>問題ID:</label>
        <input
          id="question_id"
          name="question_id"
          type="text"
          ...
          placeholder="問題IDを入力してください"
          style={{ flex: 1, minWidth: 200 }}
        />
        <button
          type="button"
          onClick={fetchQuestionById}
          disabled={fetchingQuestions || !form.question_id.trim()}
          style={{
            padding: '8px 16px',
            backgroundColor: fetchingQuestions || !form.question_id.trim() ? '#ccc' : '#1976d2',
            color: '#fff',
            border: 'none',
            borderRadius: 4,
            cursor: fetchingQuestions || !form.question_id.trim() ? 'not-allowed' : 'pointer',
            fontWeight: 500
          }}
        >
          {fetchingQuestions ? '取得中...' : '問題取得'}
        </button>
        <button
          type="button"
          ...
          style={{
            padding: '8px 16px',
            backgroundColor: fetchingQuestions ? '#ccc' : '#28a745',
            color: '#fff',
            border: 'none',
            borderRadius: 4,
        ...
        >
          {fetchingQuestions ? '取得中...' : 'ランダム取得'}
        </button>
      </div>
      
      {/* Debug URL display */}
      {form.question_id && (
        <div style={{ fontSize: '0.8em', color: '#666', marginBottom: 16, fontFamily: 'monospace', padding: '8px', backgroundColor: '#f0f0f0', borderRadius: 4 }}>
          <b>Debug URL:</b> {SHOW_QUESTION_API}/{form.question_id.trim()}
        </div>
      )}
      
      {/* Form for question info (read-only) and student details */}
      <form className="eiken-form" onSubmit={e => e.preventDefault()} aria-label="集中テストフォーム-1">
      ...
      {/* Second form: student_answer and submission */}
      <form className="eiken-form" onSubmit={handleSubmit} aria-label="集中テストフォーム-2">
...
export default IntensiveTestJa
